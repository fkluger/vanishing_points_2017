...
def pdf_params(cnn_response, confidence=1.282):
    ...
    sigma = np.pi / (confidence*A)

    alphas = np.linspace(-(A-1.0)/A*np.pi/2, (A-1.0)/A*np.pi/2, A)
    alphas = np.matlib.repmat(alphas, B, 1)
    betas = np.linspace(-(B-1.0)/B*np.pi/2, (B-1.0)/B*np.pi/2, B)
...
def pdf_grid(cnn_response, N=50):
    ...
    X = np.arange(-np.pi/2, np.pi/2, np.pi*1.0/Av)
    Y = np.arange(-np.pi/2, np.pi/2, np.pi*1.0/Bv)
    X, Y = np.meshgrid(X, Y)

    aview = np.linspace(-np.pi/2+1.0/Av, np.pi/2, Av)
    aview = np.matlib.repmat(aview, Bv, 1)
    bview = np.linspace(-np.pi/2+1.0/Bv, np.pi/2, Bv)
